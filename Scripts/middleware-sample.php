...
class MiddlewareA implements Middleware
{
    public function handle(string $request, callable $next): string
    {
        ...
        echo "A REQUEST = >" . $request . PHP_EOL;
        // var_dump($next);

        $response = $next($request);

        echo "A RESPONSE = >" . $request . PHP_EOL;
        // 後処理
...
class MiddlewareB implements Middleware
{
    public function handle(string $request, callable $next): string
    {
        ...
        $request = $request . $preprocessMessage;
        echo "B REQUEST = >" . $request . PHP_EOL;
...
class MiddlewareC implements Middleware
{
    public function handle(string $request, callable $next): string
    {
        ...
        echo "C REQUEST = >" . $request . PHP_EOL;
        $response = $next($request);
...
// これは、ネストされた長い関数コールを持つ関数スタックのように動作します。
// 先入れ先出し (FIFO) なので、$middlewares配列を逆順にします。ミドルウェアは、元の順番に従って順次実行されます。
foreach ($middlewares as $middleware) {
    echo $middleware::class . PHP_EOL;
